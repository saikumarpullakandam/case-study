...
public class ReservationServiceImpl implements ReservationService {
	...
	public String bookRoom(Reservation res) {
		Room room= restTmp.getForObject("http://localhost:8083/Room/getRoom/"+res.getRoomId(), Room.class);
		if(room.isAvailable()) {
			repo.save(res);
			room.setAvailable(false);
			restTmp.put("http://localhost:8083/Room/updateRoom", room);
	...
	public String deleteRes(int id) {
		...
		Room room= restTmp.getForObject("http://localhost:8083/Room/getRoom/"+res1.getRoomId(), Room.class);
